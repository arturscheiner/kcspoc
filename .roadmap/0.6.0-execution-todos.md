...
- [ ] E-093: Implement `--serve` (alias `-p`) flag parsing in `reports_controller.sh`
- [ ] E-094: Create `service_report_serve()` to orchestrate `index.html` generation and `http.server` startup
- [ ] E-095: Implement high-fidelity `index.html` generator in the View layer
- [ ] E-096: Update locale help text for `reports` command
