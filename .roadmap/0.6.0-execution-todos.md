...
## 🏷️ Extra-Packs Resource Labeling & State Accuracy
...
- [x] E-109: Implement `model_state_is_actually_installed` in `state_model.sh` to reconcile state file against real cluster presence (calls `_service_extra_is_installed` per entry)
- [x] E-110: Update `extras --list` view to show a reconciled status: distinguish "tracked+installed", "tracked+missing", and "untracked+present" states
