...
## 🧩 Extensible Component Architecture (Extras)
...
## 🛠️ CLI Ergonomics & UX Improvements
...
## ⚙️ Configuration Wizard UX
- [ ] E-058: Analyze current config wizard flow and define logical grouping (Localization, Cluster, Product, Networking, Registry, Secrets, Diagnostics)
- [ ] E-059: Refactor `_config_controller_wizard` in `config_controller.sh` to implement the new grouped order
- [ ] E-060: Update step numbering and UI descriptions in `config_view.sh` to match the new logical sections
- [ ] E-061: Verify that existing configurations are correctly loaded and preserved during the reordered wizard flow

## 🤖 AI Capabilities Integration (Ollama)
- [ ] E-062: Update `config_model.sh` to support `OLLAMA_ENDPOINT` and `OLLAMA_MODEL` persistence
- [ ] E-063: Integrate AI configuration prompts (Endpoint, Default Model) into the config wizard
- [ ] E-064: Implement `--ai-model` CLI flag in `base_controller.sh` and relevant sub-controllers
