...
view_destroy_safety_check() {
    ...
    view_ui_section_header "$MSG_DESTROY_TITLE"
    echo -e "${BRIGHT_RED}${BOLD}${ICON_WARN} $MSG_DESTROY_WARN_TITLE${NC}"
    echo -e "${BRIGHT_YELLOW}$MSG_DESTROY_WARN_DESC${NC}"
    echo ""
    
    echo -e "${DIM}$MSG_DESTROY_PROMPT${NC}"
    echo -e "${BRIGHT_CYAN}${BOLD}$required_phrase${NC}"
    echo ""
    echo -ne "${ICON_QUESTION} > "
    read -r user_input
    
    if [ "$user_input" != "$required_phrase" ]; then
        echo -e "\n${BRIGHT_RED}$MSG_DESTROY_CANCEL${NC}"
        return 1
    fi
    return 0
}
...
view_destroy_start_msg() {
    echo -e "${BRIGHT_YELLOW}${ICON_GEAR} $MSG_DESTROY_START${NC}"
    view_ui_line
}
...
view_destroy_infra_header() {
    echo -e "\n${BRIGHT_RED}${BOLD}=== $MSG_DESTROY_DEPS_TITLE ===${NC}"
}

view_destroy_success() {
    echo -e "\n${BRIGHT_GREEN}${BOLD}${ICON_OK} $MSG_DESTROY_SUCCESS${NC}"
    echo -e "${DIM}$MSG_DESTROY_HINT${NC}"
}

view_destroy_infra_skipped() {
    echo -e "${BRIGHT_CYAN}$MSG_DESTROY_DEPS_SKIPPED${NC}"
}
